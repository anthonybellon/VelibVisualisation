...
with open(
    get_absolute_path("../../data/do_not_touch/weekly_velib_data/weekly_velib_data.json")
) as file:
    velib_data_single_time = json.load(file)

with open(get_absolute_path("../../data/velib-disponibilite-en-temps-reel.json")) as file:
...
print(
    f"Number of stations in velib-disponibilite-en-temps-reel.json: {len(station_codes_real_time)}"
)

# Find stations that are in real-time data but not in single time data
missing_stations = [
    station
    for station in velib_disponibilite_en_temps_reel
    if station["stationcode"] not in station_codes_single_time
]
