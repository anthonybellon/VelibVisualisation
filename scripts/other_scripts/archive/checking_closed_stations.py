...
for record in data:
    try:
        ...
        if (
            fields.get("is_renting") != "OUI"
            or fields.get("is_installed") != "OUI"
            or fields.get("is_returning") != "OUI"
        ):
...
print(
    f"Last update date: {last_update_date.strftime('%Y-%m-%d %H:%M:%S') if last_update_date else 'No valid date found'}"
)
