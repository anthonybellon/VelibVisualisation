...
from utils import (
    create_features,
    get_absolute_path,
    report_infinite_and_large_values,
    save_feature_names,
)
...
def preprocess_data():
    ...
    if bike_data.empty:
        raise ValueError(
            "The concatenated bike data is empty. Please check the JSON files and their loading process."
        )
    ...
    bike_data["lat"] = (
        bike_data["coordonnees_geo"]
        .apply(lambda x: x.get("lat", np.nan) if x else np.nan)
        .astype("float32")
    )
    bike_data["lon"] = (
        bike_data["coordonnees_geo"]
        .apply(lambda x: x.get("lon", np.nan) if x else np.nan)
        .astype("float32")
    )
